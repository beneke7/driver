...
def _run_case(
    *,
    ...
    alternate_train_path = _repo_path(alternate_train_file)
    train_tokens = (
    ...
    candidate = _train_branch(
        ...
        campaign=campaign,
        target=target,
        train_values=alternate_train_values,
        validation_values=validation_values,
        checkpoint_data_sha=checkpoint_data_sha,
        config_sha=config_sha,
        data_source=alternate_train_file,
        data_source_sha=alternate_data_sha,
        start_cursor=0,
        device=device,
    )
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    output_manifest = {
        ...
        "oracle_only": True,
        "hypothesis": "switching among the two local training corpora can improve durable capability at equal token cost",
        "contract": {
